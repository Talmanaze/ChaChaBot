...
exports.run = (client, connection, P, message, args) => {
  ...
  try {
    ...
    message.channel
      .send(`data received! loading... ${shakey}`)
      .catch(console.error);
    ...
    if (b_shakeVal > b_randomShake[0]) {
      ...
      if (b_shakeVal > b_randomShake[1]) {
        ...
        if (b_shakeVal > b_randomShake[2]) {
          message.channel
            .send(
              `......it shakes three times... (so exciting!! :fingers_crossed:)`
            )
            .catch(console.error);
          if (b_shakeVal > b_randomShake[3]) {
            ...
            message.channel
              .send(`Nooo, the ${pokeName} broke free! It was so close, too...`)
              .catch(console.error);
          }
        } else {
          message.channel
            .send(`Argh, the ${pokeName} got out!`)
            .catch(console.error);
        }
      } else {
        message.channel
          .send(`Oh no, the ${pokeName} broke free!`)
          .catch(console.error);
      }
    } else {
      message.channel
        .send(`Drat! ${pokeName} broke free!`)
        .catch(console.error);
    }
  } catch (error) {
